			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_FILE_CLEARLOG:
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << STATUS_SERVER_NAME << " - Version " << STATUS_SERVER_VERSION << "." << std::endl;
						std::cout << "A server developed by Tryller." << std::endl;
						std::cout << "Visit: https://github.com/tryller/crystalserver." << std::endl << std::endl;
					}
					break;
...
int32_t WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int32_t WindowStyle)
{
	...
	if(!RegisterClassEx(&wincl))
		return 0;
	gui.m_mainWindow = CreateWindowEx(0, "crystalserver_gui", STATUS_SERVER_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 707, 453, HWND_DESKTOP, NULL, hInstance, NULL);
